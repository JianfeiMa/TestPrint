...
//        this.setBounds((int) ((SystemProperties.SCREEN_WIDTH - 800) / 2), (int) ((SystemProperties.SCREEN_HEIGHT - 600) / 2), 800, 600);
    ...
    public int print(Graphics g, PageFormat pf, int page) throws PrinterException {
        System.out.println("print方法执行了:g->" + g.toString() + ";pf->" + pf.toString() + ";page->" + page);
    ...
    private void drawCurrentPageText(Graphics2D g2, PageFormat pf, int page) {
        System.out.println("drawCurrentPageText方法开始执行page=>" + page);
        Font f = area.getFont();
        String s = getDrawText(printStr)[page];
        System.out.println("获得第一页的文本->" + s);
        String drawText;
        float ascent = 8;
        int k, i = f.getSize(), lines = 0;
        while (s.length() > 0 && lines < 54) {
            k = s.indexOf('\n');
            System.out.println("换行的索引下标->" + k);
            if (k != -1) {
                lines += 1;
                drawText = s.substring(0, k);
                System.out.println("如果换行的索引下标不等于-1的时候的文本->" + drawText);
                ...
                lines += 1;
                drawText = s;
                System.out.println("换行索引下标等于-1的时候的文本->" + drawText);
                g2.drawString(drawText, 0, ascent);
                s = "";
            }
        }
